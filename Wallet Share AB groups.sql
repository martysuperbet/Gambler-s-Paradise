...
segment as (
    ...
    where is_valid = 1 
        ...
        and segmentation_month in ('2026-01-01', '2026-02-01')
),
...
hv_segment_esi as (
    ...
    where ts.player_value_tier = 'High Value'
),
...
player_esi as (
    ...
    left join value_mapping vm
        on ts.accepted_dt_local = vm.date_value
    where ts.player_value_tier in ('High Value', 'Medium Value')
...
player_latest_tier as (
    ...
    join segment s
        ...
        and s.segmentation_month = '2026-02-01'
    where s.player_value_tier in ('High Value', 'Medium Value')
),
...
wallet_sharing_flags as (
    select
        ...
        -- HV conditions: only populated for HV players
        case when pq.player_value_tier = 'High Value' 
              and pq.player_esi_high < hs.hv_segment_esi_high 
             then 1 else 0 end as hv_meets_high_condition,
        case when pq.player_value_tier = 'High Value' 
              and (pq.player_esi_low >= hs.hv_segment_esi_low 
                or pq.player_esi_mid >= hs.hv_segment_esi_mid)
             then 1 else 0 end as hv_meets_mid_low_condition,
...
eligible_players as (
    ...
    where player_value_tier = 'Medium Value'
        ...
        and player_esi_high <= 0.4184
        and (player_esi_mid >= 0.4372 or player_esi_low >= 0.2317)
),
...
where final_rn <= 918  -- 900 per group = 1800 total
...
select
    sgm.player_id,
    null                            as stake_quintile,
    null                            as activity_band,
    null                            as active_days,
    null                            as low_activity_flag,
    null                            as ab_group,
    
from PROD.BI.VW_PLAYER_MONTHLY_SEGMENTATION_ALL sgm
where segmentation_month in ('2026-02-01')
    and ACTUAL_OFFICIAL_SEGMENTATION = 'Medium Value'
